...
use esp_idf_svc::{
    hal::{
        ...
        gpio::{Gpio8, Gpio9, Gpio16, Gpio45},
        i2c::{I2cConfig, I2cDriver},
        i2s::{self, I2sDriver, config::StdConfig},
        prelude::Peripherals,
    },
    http::{
        self,
...
use symphonia::core::{
    audio::RawSampleBuffer, formats::FormatOptions, io::MediaSourceStream, meta::MetadataOptions,
    probe::Hint,
};
...
async fn main(spawner: Spawner) -> ! {
    ...
    let i2c: Mutex<I2cDriver<'static>> =
    ...
    loop {
        let poll_buttons = speaker_control(
            &mut speaker_controller,
            current_station,
            &mut adc1,
            &mut adc_pin,
        );
...
///
///### Notes
///* You might want to call [buttons::diagnose],
...
async fn speaker_control<I2C: I2c>(
    speaker_controller: &mut SpeakerDriver<I2C>,
    current_station: usize,
    adc1: &mut esp_idf_svc::hal::adc::ADC1,
    adc_pin: &mut esp_idf_svc::hal::gpio::Gpio5,
) -> usize {
    ...
    loop {
        ...
        match adc.read(&mut adc_pin).unwrap() {
            ...
            res => log::error!(
                "Could not definitively identify which button was pressed. Got value {res}"
            ),
        }
    }
}
...
async unsafe fn stream(
    raw_client: *mut Client<EspHttpConnection>,
    current_station: usize,
    i2s0: &mut i2s::I2S0,
    bclk: &mut Gpio9,
    dout: &mut Gpio8,
    ws: &mut Gpio45,
    mclk: &mut Option<Gpio16>,
) {
    ...
    log::info!(
        "The metadata is {:#?}",
        probe_res.format.metadata().skip_to_latest()
    );
    log::info!(
        "Additional metadata is {:#?} and the tracks are {:#?}",
        probe_res.metadata.get(),
        probe_res.format.tracks()
    );

    let first_supported_track = probe_res
        .format
        .tracks()
        .iter()
        .find(|t| {
            (t.codec_params.codec == symphonia::core::codecs::CODEC_TYPE_MP3)
    ...
    log::info!(
        "the id of first supported track is {:#?} and the codec param are {:#?}",
        first_supported_track.id,
        first_supported_track.codec_params
    );

    let sample_rate = if let Some(sample_rate) = first_supported_track.codec_params.sample_rate {
    ...
    */
    let i2s_config = StdConfig::new(
        ...
        i2s::config::StdSlotConfig::philips_slot_default(
            i2s::config::DataBitWidth::Bits24,
    ...
    let mut i2s_driver =
        I2sDriver::new_std_tx(i2s0, &i2s_config, bclk, dout, mclk.as_mut(), ws).unwrap();
    ...
    let decoder_options: symphonia::core::codecs::DecoderOptions =
                ...
                match decoder.decode(&packet) {
                    Ok(decoded_audio) => {
                        ...
                        if decoded_buf.is_none() {
                            ...
                            decoded_buf =
                                Some(RawSampleBuffer::<symphonia::core::sample::i24>::new(
                                    duration, spec,
                                ));
                        }
                    ...
                    Err(symphonia_Error::DecodeError(_)) | Err(symphonia_Error::IoError(_)) => {
                        continue;
                    }
            ...
            Some(buf) => {
                if preload {
                    ...
                    if new_loaded_bytes < data.len() {
                        ...
                        i2s_driver
                            .write_all_async(&data[new_loaded_bytes..])
                            .await
                            .unwrap();
                    }
                } else {
...
fn connect(
    ...
    loop {
        if connection_attempts == 0 {
            panic!(
                "Failed to connect to {} with {} retries",
                STATION_URLS[current_station], RETRIES
            );
        }
        ...
        let client: &'static mut Client<EspHttpConnection> = unsafe { &mut *raw_client };
        ...
        */
        client
            .connection()
            .modify_get_request(STATION_URLS[current_station])
            .unwrap();
        let request = embedded_svc::http::client::Request::wrap(client.connection());
...
async fn blink(
    mut led_driver: LedDriver<MutexDevice<'static, I2cDriver<'static>>>,
...
mod audio_stream {
    ...
    pub fn new_stream<'a>(
        ...
        ReadOnlySource::new(StreamSource {
            inner: Mutex::new(StreamSourceInner { inner: response }),
        })
    }
