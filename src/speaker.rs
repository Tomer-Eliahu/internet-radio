...
impl<I2C: I2c> SpeakerDriver<I2C> {
    ...
    pub fn build(i2c: I2C, pa_ctrl_pin: Gpio48, initial_vol: Volume) -> Self {
        ...
        speaker_driver
            .i2c
            .write(Self::I2C_ADDR, &[Register::ClockFactors as u8, 0b00011000])
            .unwrap();
        ...
        speaker_driver
            ...
            .write(
                Self::I2C_ADDR,
                &[
                    Register::DacEqualizer as u8,
                    Register::DacEqualizer.default(),
                ],
            )
            .unwrap();
        ...
        speaker_driver
            .i2c
            .write(Self::I2C_ADDR, &[Register::SysPwrMgt as u8, 0b00110101])
            .unwrap();

        //Turn on relevant clocks
        speaker_driver
            .i2c
            .write(Self::I2C_ADDR, &[Register::ClockManager as u8, 0b10110101])
            .unwrap();

        //Power up DAC
        speaker_driver
            .i2c
            .write(Self::I2C_ADDR, &[Register::SysEnableDac as u8, 0x1])
            .unwrap();
        ...
        speaker_driver
            ...
            .write(
                Self::I2C_ADDR,
                &[Register::SysEnableSpeakerDrive as u8, write_in],
            )
            .unwrap();
    ...
    fn codec_software_reset(&mut self) {
        ...
        self.i2c
            .write(
                Self::I2C_ADDR,
                &[Register::Reset as u8, Register::Reset.default()],
            )
            .unwrap();
        ...
        self.i2c
            .write(Self::I2C_ADDR, &[Register::Reset as u8, (1 << 7)])
            .unwrap();
    }
    ...
    pub fn mute(&mut self) {
        ...
        self.i2c
            .write_read(Self::I2C_ADDR, &[Register::DacMute as u8], &mut reg_value)
            .unwrap();

        let write_in = reg_value[0] | 0b1110_0000;

        self.i2c
            .write(Self::I2C_ADDR, &[Register::DacMute as u8, write_in])
            .unwrap();
    }
    ...
    pub fn set_vol(&mut self, vol: Volume) {
        ...
        self.i2c
            .write(Self::I2C_ADDR, &[Register::DacVol as u8, desired])
            .unwrap();
...
impl Register {
    ...
    const fn default(&self) -> u8 {
        match *self {
            ...
            Register::DacMute
            | Register::DacVol
            | Register::ClockManager
            | Register::ClockFactors => 0,
